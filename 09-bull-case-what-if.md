...
| Grok reaches the frontier by September 2026 | **MEDIUM** | Grok 4.3 is #4 (~8 pts behind Opus 4.8). xAI now claims Grok 4.5 / V9 (1.5T, Cursor-data) is "close to, perhaps exceeding Opus," but that is an internal eval on a private-beta model with no public release and no independent benchmark. The 6T Grok 5 already slipped twice (~33% odds before Jun 30). A *verified* frontier result, not an xAI claim, is what this needs. |
...
- **Grok frontier by Sept:** needs a Grok model (the 6T Grok 5, or the 1.5T Grok 4.5 / V9 now in private beta) reaching the public in usable, **independently-verified** form by ~July-Aug at or above Opus 4.8 / GPT-5.5, which are themselves iterating. xAI's own "near/above Opus" eval on Grok 4.5 does not count until a third party confirms it. *Breaks if:* it stays in beta, the public release slips, or independent benchmarks land below frontier (the 6T Grok 5 already missed Q1 and Q2; ~33% odds before Jun 30).
